...
def tree_size(root):
    ...
    >>> html = "<p><font><font>^ </font><a href='#top'>\
            <font>Return to the top</font></a></font><br><font>--</font> </p>"
...
class SimpleTreeMatch(object):
    ...
    Simple Tree Match.
    """
    def __init__(self, ger_root=_get_root,
                 get_children_count=_get_children_count, get_child=_get_child):
...
class SimpleTreeAligner(object):
    ...
    def align(self, l1, l2):
        ...
        for i in xrange(1, len(matrix)):
            for j in xrange(1, len(matrix[0])):
                ...
                alignment_matrix[i-1][j-1] = self.single_align(l1[i - 1],
                                                               l2[j - 1])
    ...
    def single_align(self, t1, t2):
        ...
        >>> t1 = fragment_fromstring("<div> <h1></h1> <h2></h2> <h5></h5> \
                                     </div>")
        >>> t2 = fragment_fromstring("<div> <h2></h2> <h3></h3> <h4></h4> \
                                     <h5></h5> </div>")
        ...
        for i in xrange(1, len(matrix)):
            for j in xrange(1, len(matrix[0])):
                if matrix[i][j - 1] > matrix[i - 1][j]:
                    ...
                    matrix[i][j] = matrix[i - 1][j]
                    trace[i - 1][j - 1] = TreeAlignment.TRACE_UP
                alignment_matrix[i - 1][j - 1] = self.single_align(
                    self.get_child(t1, i - 1), self.get_child(t2, j - 1))
...
def find_subsequence(iterable, predicate):
    ...
    for example:
    >>> find_subsequence([1, 10, 2, 3, 5, 8, 9], \
                         lambda x: x in [1, 0, 2, 3, 4, 5, 8, 9])
    [[1], [2, 3, 5, 8, 9]]

    >>> find_subsequence([1, 10, 2, 3, 5, 8, 9],\
                         lambda x: x not in [1, 0, 2, 3, 4, 5, 8, 9])
    [[10]]
...
class PartialTreeAligner(object):
    ...
    def align(self, l1, l2):
        ...
        >>> t2 = fragment_fromstring("<p> <b></b> <c></c> <d></d> <h></h> \
                                     <k></k> </p>")
        >>> _, _, mapping = pta.align([t1], [t2])
        >>> [e.tag for e in t1]
        ['x', 'b', 'c', 'd', 'h', 'k']
        ...
        for l in unaligned_elements:
            ...
            if prev_sibling is None:
                if next_sibling is not None:
                    # leftmost alignment
                    next_sibling_match = reverse_aligned.get(next_sibling,
                                                             None)
                ...
                if (prev_sibling_match is not None and
                        next_sibling_match is not None):
    ...
    def find_unaligned_elements(self, aligned, elements):
        ...
        >>> aligned = dict((align.first, align.second)\
                           for align in sta.align([t1], [t2]).subs)
        >>> unaligned = pta.find_unaligned_elements(aligned, [t2])
        >>> [[e.tag for e in l] for l in unaligned]
        [['h3', 'h4'], ['h6']]
        """
        def predicate(x): return x not in aligned.values()
        unaligned = []
